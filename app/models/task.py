...
class Task(Base):
    ...
    def __init__(self, model_id=None, model_name=None, **kwargs):
        self.model_id = hex(int(dt.now().strftime("%Y%m%d%H%M%s"))) if model_id is None else model_id
        if model_name is None:
            HTTPException(status_code=404, detail="model name have to be set. ")
